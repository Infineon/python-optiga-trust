...
def to_json():
    ...
    for oid in opt.key_id_values:
        logger.debug("Reading: {0}".format(hex(oid)))
        key = optiga.Object(oid)
        raw_meta = key.read_raw_meta().hex()
        if len(raw_meta) == 0:
            continue
        output[hex(oid)[2:]] = {"metadata": raw_meta, "pretty_metadata": key.meta}
        del key
    ...
    for oid in opt.object_id_values:
        ...
        try:
            ...
            logger.error("Data in {0} is not readable - skip.".format(hex(oid)))
            data = ""
        if len(raw_meta) == 0:
            continue
        output[hex(oid)[2:]] = {"metadata": raw_meta, "pretty_metadata": key.meta, "data": data}
        del key
...
def from_json(data=None):
    ...
    for oid, content in supermeta.items():
        ...
        logger.info("Reading {0}".format(oid))
        ...
        try:
            ...
            logger.warning("Warning. Failed to update {0} metadata. Skipping".format(oid))
        else:
            if metadata_changed:
                obj.meta = old_meta
