...
optiga_lib_status_t optiga_comms_open(optiga_comms_t *p_ctx)
{
  ...
  ((com_context_t *)p_ctx->p_comms_ctx)->fd = open_serial_port(((com_context_t *)p_ctx->p_comms_ctx)->com_port, 115200);
                                            ...
                                            uint16_t *p_rx_data_len)
{
  ...
  do
  {
    ...
    number_of_bytes_written = write_port(((com_context_t *)p_ctx->p_comms_ctx)->fd, max_transmit_frame, tx_data_length + 8);
