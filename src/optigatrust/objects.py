...
class X509(optiga.Object):
    ...
    def _update(self, cert: str or bytes or bytearray):
        ...
        _supported_objects = (
            oids.IFX_CERT.value,
            oids.USER_CERT_1.value,
            oids.USER_CERT_2.value,
            oids.USER_CERT_3.value,
            oids.TRUST_ANCHOR_1.value,
            oids.TRUST_ANCHOR_2.value,
            oids.DATA_SLOT_1500B_0.value,
        ...
        logger.info("Certificate without encoding #1 {0}".format(list(der_cert)))
        logger.info("Certificate without encoding #2 {0}".format(list(l1_der_cert)))
        logger.info("Certificate without encoding #3 {0}".format(list(l2_der_cert)))
        logger.info("Certificate without encoding #4 {0}".format(list(l3_der_cert)))
    ...
    def _read(self, to_pem=False):
        ...
        _supported_objects = (
            ...
            oid.USER_CERT_2.value,
            oid.USER_CERT_3.value,
            oid.TRUST_ANCHOR_1.value,
            oid.TRUST_ANCHOR_2.value,
            oid.DATA_SLOT_1500B_0.value,
        ...
        logger.debug(list(der_cert))

        if len(der_cert) == 0:
            raise ValueError("Certificate Slot {0} is empty".format(self.id))
